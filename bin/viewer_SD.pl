...
#return total *.SD files
our $SD_names_pointer=E_RNA::files_list($variables{result_dir}, 'file_names', 'SD');
our @total_SD_names=@$SD_names_pointer;
...
$window->set_title('SD Viewer for viewing sequencing depth');
...
sub launch{
		...
		#$variables{SD_compression}=$cb_entry_2->get_active_text();

		#return index_names of selected *.SD files
		foreach my $sample_name(@selected_names){
			foreach my $SD_name(@total_SD_names){
				if($SD_name=~/$sample_name/){
					my $index_name=$SD_name;
					$index_name=~s/$sample_name\_|\.SD$//g;
					$selected_SD{$index_name} .= ','.$SD_name;
					$selected_SD{$index_name} =~s/^,//;
				}
			}
		}
	...
	while(1){
		...
		if ( $shash{'go'} == 1 ){#2
			foreach my $sample_name(@sub_selected_names){
				#return all *.SD files
				my @SD_names=grep(/$sample_name/, @total_SD_names);
				my $SD_num=@SD_names;
				# Create a communication bridge with R and start R
				my $R=Statistics::R->new();
				my $plot_file=$variables{result_dir}.'/SD_seperated_'.$sample_name.'.jpg';
				$R->set('plot_file', $plot_file);
				$R->set('SD_num', $SD_num);
				$R->run(	qq'jpeg(plot_file, units="mm", height=60*SD_num, width=120,res=200, pointsize=8)',
						q'par(mfrow=c(SD_num,1), mar=c(4,4,2,2))',
						);
				foreach my $SD_name(@SD_names){
					my $index_name=$SD_name;
					$index_name=~s/$sample_name\_|\.SD//g;
					my $SD_file=$variables{result_dir}.'/'.$SD_name;
					$R->set('SD_file', $SD_file);
					$R->set('sample_name', $sample_name);
					$R->set('index_name', $index_name);
					$R->run(	q'df<-read.delim(SD_file, header=F, sep="\t")',
							q'plot(x=round(df[,1]/1e6, 1), y=df[,2], cex=0.5, type="l", xlab="The number of raw reads (million)", ylab="The number of references",  main=paste("Sequencing depth of ", sample_name, " against ", index_name) )',
							q'abline(0.95*max(df[,2]),0)',
							);
				}
				$R->run(q'dev.off()');
				last if $shash{'go'} == 0;
...
sub combined_work{
	...
	while(1){
		return if $shash{'die'} == 1;
		if ( $shash{'go'} == 1 ){#2
			foreach my $index_name(keys %selected_SD){#3 index circyling
				# Create a communication bridge with R and start R
				my $R=Statistics::R->new();
				my $plot_file=$variables{result_dir}.'/SD_combined_'.$index_name.'.jpg';
				$R->set('plot_file', $plot_file);
				$R->run(	qq'jpeg(plot_file, units="mm", height=100, width=150,res=200, pointsize=8)',	);
				
				my $num=1;
				my @selected_samples;
				my @SD_names=split(',', $selected_SD{$index_name});
				foreach my $SD_name(@SD_names){ #SD names circyling
					my $sample_name=$SD_name;
					$sample_name=~s/\_$index_name|\.SD$//g;
					push(@selected_samples, $sample_name);
					my $SD_file=$variables{result_dir}.'/'.$SD_name;
					my $selected_num=@SD_names;
					$R->set('SD_file', $SD_file);
					$R->set('sample_name', $sample_name);
					$R->set('index_name', $index_name);
					$R->set('selected_num', $selected_num); 
					$R->set('num', $num); 
					if($num==1){
						$R->run(	
						q'df<-read.delim(SD_file, header=F, sep="\t")',
						q'xlim_max=2*round( max(df[,1])/1e6 )',
						q'ylim_max=2*max(df[,2])',
						q'plot(x=round(df[,1]/1e6, 1), y=df[,2], cex=0.5, col=num, xlim=c( 0, xlim_max), ylim=c( 0, ylim_max), type="l", xlab="The number of raw reads (million)", ylab="The number of references", main=paste("Sequencing depth of ", selected_num, " samples against ", index_name) )',
						);
					}
					else{
						$R->run(	q'df<-read.delim(SD_file, header=F, sep="\t")',
								q'points(x=round(df[,1]/1e6, 1), y=df[,2], cex=0.5, col=num, type="l" )',
							);
					}
					$num++;
					last if $shash{'go'} == 0;
				...
				$R->set('files', \@selected_samples); 
				#print "$index_name=@selected_samples\n";
				$R->run(q'legend("bottomright", legend=files, lwd=1, col=seq(from=1, to=selected_num, by=1) )',);
				$R->run(q'dev.off()',
					);
			} #3 index circyling
			$shash{'go'} = 0;   #turn off 
...
sub create_view {
	...
	if ($variables{plot_mode}=~/Seperated/i){
		$buffer->insert($iter, "#######SD pictures are plotted seperatedly.\n\n\n");
		foreach my $sample_name(@selected_names){
			my $plot_file=$variables{result_dir}.'/SD_seperated_'.$sample_name.'.jpg';
			if(-f $plot_file){
				$buffer->insert($iter, "SD plot of $sample_name ($plot_file):\n");
		...
		$buffer->insert($iter, "#######SD pictures are plotted combined based on index name.\n\n\n");
		foreach my $index_name(keys %selected_SD){
			my $plot_file=$variables{result_dir}.'/SD_combined_'.$index_name.'.jpg';
			print "###$index_name\n";
			if(-f $plot_file){
				$buffer->insert($iter, "SD plots against $index_name ($plot_file):\n");
				my $pix_buffer=Gtk2::Gdk::Pixbuf->new_from_file($plot_file);
				$buffer->insert_pixbuf($iter, $pix_buffer);
				$buffer->insert($iter, "\n");
			}
